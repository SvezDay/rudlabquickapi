...
module.exports.getDetail = (tx, uid, idx_uuid)=>{
  return new Promise((resolve, reject)=>{
    ...
    .then(() => {return miscellaneousReq.access2Index(tx, uid, idx_uuid) })
    .then(() => {return graphReq.getGraph(tx, uid, idx_uuid) }).then(result => graph=result)
    .then(() => {
    ...
    .catch(err =>{console.log(err); reject({status: err.status || 400, mess: err.mess || 'free/graph-detail.ctrl.js/getDetail'}); })
  })
};
